...
# Para usar a função "column_repetition" sem recriá-la aqui, vamos instanciar um objeto de classe "Card"
...
for game in dtb:
    ...
    obj.game = game
    # Após cada análise, se insere em "rank" o resultado se o jogo possui colunas repetidas
    rank.append(f"{obj.column_repetition()['empty']}")

# a_b
ten_last_columns_report = [
...
if __name__ == '__main__':
    ...
    print(f'Quantos jogos têm colunas todas diferentes? {relative_frequence_true} jogos [{percentage_for_true}]')
    print(f'Quantos jogos têm colunas iguais?           {relative_frequence_false} jogos [{percentage_for_false}]')

    # a_c: Se o jogo não possuir colunas iguais (True), mostrar que o jogo foi aprovado, caso contrário, mostrar vazio
    print('\nRelatório dos 10 últimos jogos (linhas iguais)')
    for index, game in enumerate(ten_last):
        print(*[f'[aprovado] {game}' if ten_last_columns_report[index] else '           '])
